...
import asyncio
import websockets
from config_server import ServerConfig as Config, ModelPaths, __version__
...
BASE_DIR = os.environ.get("CAPSWRITER_HOME", "").strip() or os.path.dirname(__file__)
...
def init():
    ...
    logger.info(f"语音模型类型: {Config.model_type}")
    logger.info(f"语音模型目录: {ModelPaths.model_dir}")

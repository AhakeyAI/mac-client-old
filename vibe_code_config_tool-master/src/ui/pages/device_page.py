...
class DevicePage(QWidget):
    ...
    def _setup_ui(self):
        ...
        # ========== Bridge 状态 ==========
        bridge_group = QGroupBox("Bridge 状态")
        ...
        for row, col, key, name in bridge_fields:
            lbl_name = QLabel(f"{name}:")
            lbl_name.setStyleSheet("color: #888;")
            lbl_val = QLabel("--")
            lbl_val.setStyleSheet("font-weight: bold;")
            ...
            bridge_layout.addWidget(lbl_name, row, col * 2)
            bridge_layout.addWidget(lbl_val, row, col * 2 + 1)
            self._bridge_labels[key] = lbl_val
    ...
    def update_bridge_runtime(self, info: dict):
        """更新 Bridge 运行状态显示"""
        mapping = {
            "status": "status",
            "mode": "mode",
        ...
        for key, info_key in mapping.items():
            val = info.get(info_key, "--")
            self._bridge_labels[key].setText(str(val))
    ...
    def log(self, message: str, level: str = "info"):
        ...
        self.log_text.append(f'<span style="color:{color}">{ts} &gt; {safe_message}</span>')

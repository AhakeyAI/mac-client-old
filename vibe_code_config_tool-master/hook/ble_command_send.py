...
def get_config_path() -> str:
    if is_frozen() and sys.platform == "darwin":
        app_support = Path.home() / "Library" / "Application Support" / "VibeKeyboard"
        app_support.mkdir(parents=True, exist_ok=True)
        return str(app_support / "config_client.json")
    return os.path.join(os.path.dirname(get_self_path()), "config_client.json")
